...
# config/local.json 에 실제 naverMapClientId 입력

fvm flutter pub get
fvm flutter run
```

- `config/local.json` 은 gitignore 대상이라 각자 로컬에만 둡니다.
- `config/local.example.json` 을 복사해서 사용하세요.
- Naver Map Client ID가 없거나 인증 실패 시 fallback 프리뷰로 내려갈 수 있습니다.
